...
### 要確認ポイントの報告（2026-07確定）

- サブエージェント・メインエージェントが**公式サイトを特定できない**（SNS/Linktreeのみ等）・**監視URLの妥当な候補が見つからない**・**根拠を確認できず null にした重要項目がある**等、自力で解決できなかった事項が発生した場合は、黙って null のままにする/放置するのではなく、**更新処理完了時の報告に「要確認ポイント」としてまとめてユーザーに伝えること**。
- 例: 「kurayamisakaは公式サイトが見つからず、SNS・eplusのアーティストページのみで運用中」等。
- ユーザーが個別ページURL等を把握している場合があるため、判断に迷ったら独断で推測URLを使わず、要確認ポイントとして報告を優先する。
