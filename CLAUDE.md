...
> **作業前に必ず `git checkout dev && git pull origin dev` を実行してから作業を開始すること。**
> **必ず validate.py を通過してから push すること。**

---

## dev/main ブランチ運用（2026-07確定・最重要）

アプリが既にリリース済みのため、**本番ユーザーが今まさに読んでいる配信（`https://live-ticket-data.pages.dev` = `main`ブランチ）に、検証前のデータ収集作業を直接反映してはならない。**

- **データ収集・更新作業は全て `dev` ブランチで行う**（本ファイルの各ワークフローは全て `git push origin dev` を使う）
- `dev` ブランチは Cloudflare Pages のプレビューデプロイにより `https://dev.live-ticket-data.pages.dev` で自動配信される（本番とは別URL、追加料金なし）
- アプリ側の開発・検証ビルドは `--dart-define=DATA_BASE_URL=https://dev.live-ticket-data.pages.dev` を使うことで、本番ユーザーに影響を与えずに新しいデータを確認できる
- **`main`（本番）への反映は、`dev`での作業が一通り完了し内容を確認した後、別タスクとして明示的に行う。** 自動では反映しない。

### 本番反映（dev → main）の手順

1. `dev`ブランチの内容を確認（`python3 tools/validate.py`が通っていること、必要ならアプリの開発ビルドで動作確認）
2. **ユーザーに本番反映してよいか確認を取る**（無断でmainにマージしない）
3. 確認が取れたら:
   ```bash
   git checkout main
...
7. `git add data/artist/{id}.json data/artists.json data/manifest.json && git commit -m "update: {アーティスト名}情報更新" && git push origin dev`
...
- **情報収集の注意点（必ず守ること）:**
  ...
  - **存在を確認していないURLをパスで推測して試すことは禁止**。URLは必ず WebFetch で取得したページ内のリンクから辿ること。ページが JS レンダリングでリンクが取れない場合は WebSearch で正しいURLを探すこと。
...
3. グループごとに1組×5並列で実行:
   ...
   - `git add data/artist/... data/artists.json && git commit -m "refresh: {アーティスト名}など" && git push origin dev`
4. 全グループ完了後:
   - `python3 tools/cleanup_past.py` → `git add data/artist/*.json && git commit -m "cleanup: 終了ツアー削除" && git push origin dev`
   - `python3 tools/update_manifest.py` → `git add data/manifest.json && git commit -m "update: manifest" && git push origin dev`
...
3. グループごとに1組×5並列で実行:
   ...
   - `data/artists.json` の `lastVerifiedAt` を5組分一括更新
   - `python3 tools/validate.py`（エラーがあれば修正）
   - `git add data/artist/... data/artists.json && git commit -m "refresh: Hot tier {アーティスト名}など" && git push origin dev`
4. 全グループ完了後:
   - `python3 tools/cleanup_past.py` → `git add data/artist/*.json && git commit -m "cleanup: 終了ツアー削除" && git push origin dev`
   - `python3 tools/update_manifest.py` → `git add data/manifest.json && git commit -m "update: manifest" && git push origin dev`
...
3. グループごとに1組×5並列で実行:
   ...
   - `data/artists.json` の `lastVerifiedAt` を5組分一括更新
   - `python3 tools/validate.py`（エラーがあれば修正）
   - `git add data/artist/... data/artists.json && git commit -m "refresh: {アーティスト名}など更新" && git push origin dev`
4. 全グループ完了後:
   - `python3 tools/cleanup_past.py` → `git add data/artist/*.json && git commit -m "cleanup: 終了ツアー削除" && git push origin dev`
   - `python3 tools/update_manifest.py` → `git add data/manifest.json && git commit -m "update: manifest" && git push origin dev`
...
3. グループごとに1組×5並列で実行:
   ...
   - `python3 tools/validate.py`（エラーがあれば修正）
   - `git add data/artist/... data/artists.json && git commit -m "add: {アーティスト名}など" && git push origin dev`
4. 全グループ完了後: `python3 tools/update_manifest.py` → `git add data/manifest.json && git commit -m "update: manifest" && git push origin dev`
